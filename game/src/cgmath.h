...
struct mat4
{
	...
	static mat4 perspective( float left, float right, float top, float bottom, float dnear, float dfar ) { return mat4().set_perspective(left, right, top, bottom, dnear, dfar); }
	...
	mat4& set_perspective( float left, float right, float top, float bottom, float dnear, float dfar )
	{
		set_identity();
		_11 = 2 / (right - left);
		_22 = 2 / (top - bottom);
		_33 = 2 / (dnear - dfar);
		_14 = (left + right) / (left - right);
		_24 = (bottom + top) / (bottom - top);
		_34 = (dnear + dfar) / (dnear - dfar);
		_44 = 1;
		return *this;
	}
